...
class DispositivoRestControllerTest {
    ...
    void getAllByMarca() {
        // Arrange
        var dispositivoResponses = List.of(dispositivoResponse1);
        String marca = dispositivoResponse1.getMarca(); // Samsung
        String queryString = "?marca=" + marca;
    ...
    void getAllByTitular() {
        ...
        String titular = dispositivoResponse2.getTitular(); // Cristina
        String queryString = "?titular=" + titular;

        // Configuramos el mock: marca=null, titular="Cristina"
        ...
        // Verify
        verify(dispositivosService, only()).findAll(isNull(), eq(titular));
    }
    ...
    void getAllByMarcaAndTitular() {
        // Arrange
        var dispositivoResponses = List.of(dispositivoResponse1);
        String marca = dispositivoResponse1.getMarca();
        String titular = dispositivoResponse1.getTitular();

        String queryString = "?marca=" + marca + "&titular=" + titular;

        // Configuramos el mock: ambos parámetros
        when(dispositivosService.findAll(eq(marca), eq(titular))).thenReturn(dispositivoResponses);
        ...
        // Verify
        verify(dispositivosService, only()).findAll(eq(marca), eq(titular));
    }
