...
import es.danielmc.rest.dispositivos.services.DispositivosServiceImpl;
...
import es.danielmc.rest.titulares.services.TitularesService;
...
class DispositivosServiceImplTest {
    ...
    void findAll_ShouldReturnAllDispositivos_WhenNoParametersProvided(){
        ...
        Page<Dispositivo> expectedPage = new PageImpl<>(expectedDispositivos);
        when(dispositivoRepository.findAll(any(Specification.class), any(Pageable.class))).thenReturn(expectedPage);

        // Act
        Page<DispositivoResponseDto> actualPage =
                dispositivosService.findAll(Optional.empty(), Optional.empty(), Optional.empty(), pageable);
        ...
        verify(dispositivoRepository, times(1)).findAll(any(Specification.class), any(Pageable.class));
    }

    @Test
    void findAll_ShouldReturnDispositivosByMarca_WhenMarcaParameterProvided(){
        Optional<String> marca = Optional.of("Apple");
        List<Dispositivo> expectedDispositivo = List.of(dispositivo1);
        Pageable pageable = PageRequest.of(0, 10, Sort.by("id").ascending()); // ejemplo de creación de un objeto Pageable
        Page<Dispositivo> expectedPage = new PageImpl<>(expectedDispositivo);
        when(dispositivoRepository.findAll(any(Specification.class), any(Pageable.class))).thenReturn(expectedPage);

        // Act
        Page<DispositivoResponseDto> actualPage =
                dispositivosService.findAll(marca, Optional.empty(), Optional.empty(), pageable);
        ...
        verify(dispositivoRepository, only()).findAll(any(Specification.class), any(Pageable.class));
    }
