...
public class DispositivoRestControllerTest {
    ...
    @Test
    void getAllByMarca() {
        var tarjetasResponses = List.of(dispositivoResponse2);
        String queryString = "?marca=" + dispositivoResponse2.getMarca();
        when(dispositivosService.findAll(anyString(), isNull())).thenReturn(tarjetasResponses);
        ...
        assertThat(result)
                .hasStatusOk()
                .bodyJson().satisfies(json -> {
                    assertThat(json).extractingPath("$.length()").isEqualTo(tarjetasResponses.size());
                    assertThat(json).extractingPath("$[0]")
                            .convertTo(DispositivoResponseDto.class).isEqualTo(dispositivoResponse2);
                });
    ...
    void getAllByModelo() {
        ...
        when(dispositivosService.findAll(isNull(), anyString())).thenReturn(tarjetasResponses);
    ...
    void getAllByMarcaAndModelo() {
        ...
        when(dispositivosService.findAll(anyString(), anyString())).thenReturn(tarjetasResponses);
