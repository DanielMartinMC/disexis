package es.danielmc.rest.users.controllers;
...
import es.danielmc.rest.dispositivos.exceptions.DispositivoException;
...
import es.danielmc.rest.users.exceptions.UserNameOrEmailExists;
import es.danielmc.rest.users.exceptions.UserNotFound;
...
public class UsersRestController {
   ...
   */
  @GetMapping("/me/disositivos")
  @PreAuthorize("hasRole('USER')") // Solo los usuarios pueden acceder
  public ResponseEntity<PageResponse<DispositivoResponseDto>> getTarjetasByUsuario(
    ...
    return ResponseEntity.ok(PageResponse.of(
        dispositivosService.findByUsuarioId(user.getId(), pageable), sortBy, direction));
  }
   ...
   *
   * @param user     usuario autenticado
   * @param idDispositivo id de la tarjeta
   * @return Tarjeta
   */
  @GetMapping("/me/dispositivos/{id}")
  @PreAuthorize("hasRole('USER')") // Solo los usuarios pueden acceder
  public ResponseEntity<DispositivoResponseDto> getDispositivo(
      @AuthenticationPrincipal User user,
      @PathVariable("id") Long idDispositivo
  ) {
    log.info("Obteniendo dispositivo con id: {}", idDispositivo);
    // Solo puedo verla si es una tarjeta mía
    return ResponseEntity.ok(dispositivosService.findByUsuarioId(user.getId(), idDispositivo));
  }
   ...
   *
   * @param user   usuario autenticado
   * @param dispositivo tarjeta a crear
   * @return Tarjeta creada
   * @throws HttpClientErrorException.BadRequest si hay algún error de validación (400)
   */
  @PostMapping("/me/dispositivos")
  @PreAuthorize("hasRole('USER')") // Solo los usuarios pueden acceder
  public ResponseEntity<DispositivoResponseDto> saveDispositivo(
      @AuthenticationPrincipal User user,
      @Valid @RequestBody DispositivoCreateDto dispositivo
  ) {
    log.info("Creando dispositivo: {}", dispositivo);
    return ResponseEntity.status(HttpStatus.CREATED).body(dispositivosService.save(dispositivo, user.getId()));
  }
  ...
  @PreAuthorize("hasRole('USER')") // Solo los usuarios pueden acceder
  public ResponseEntity<DispositivoResponseDto> updateDispositivo(
      @AuthenticationPrincipal User user,
      @PathVariable("id") Long idDispositivo,
      @Valid @RequestBody DispositivoUpdateDto tarjeta) {
    log.info("Actualizando dispositivo con id: {}", idDispositivo);
    return ResponseEntity.ok(dispositivosService.update(idDispositivo, tarjeta,  user.getId()));
  }
  ...
  public ResponseEntity<Void> deleteTarjeta(
    ...
    log.info("Borrando dispositivo con id: {}", idDispositivo);
    dispositivosService.deleteById(idDispositivo,  user.getId());
    return ResponseEntity.noContent().build();
  }
